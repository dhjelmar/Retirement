...
def checkit(df, expected_savings, expected_roth, expected_ira, expected_assets_cd):
    ...
    check = all([pass_savings, pass_roth, pass_ira, pass_assets])
    breakpoint()
    print('pass?:', check)
    return check
...
check = []
test = []
cols = ['year','age','income','rmd','ira_convert','savings','roth','ira','taxable','federal','state','medicare','assets','assets_cd']
...
test.append(my.scenario('test'+str(i), spending, max_taxable, marr, roi, roi_savings, inflation, 
                            start, year, age,
                            income, ira_initial, roth_initial, savings_initial,
...
expected_assets_cd = (expected_savings + expected_roth + (1-0.24)*expected_ira)/(1+inflation)**len(test[i].df)
checki = checkit(test[i].df, expected_savings, expected_roth, expected_ira, expected_assets_cd)
...
ira_age_72 = test[i].df.loc[test[i].df.age==72,'ira'].iloc[0]
...
my.plotout(test, yvar='assets_cd', xvar='age', xlim='auto', ylim='auto')
# %%
