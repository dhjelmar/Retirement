...
def tax(year, inflation, income, lcg=0):
    ...
    federal_income, rate_federal_income = taxcalc(income, start, rate)

    # long term capital gains
    start = [0, 48351, 533401,   1E6]                                 # 2026 tax table
    start = np.array(start) * (1 + inflation)**(year - tax_table_year)
    rate  = [0,  0.15,   0.20,  0.20]
    federal_lcg, rate_federal_lcg = taxcalc(lcg, start, rate)
    ...
    state, rate_state = taxcalc(income+lcg, start, rate)

    return federal, state, rate_federal_income, rate_federal_lcg, rate_state
#%%
...
def tax_test(year=2030, inflation=0.025):
    ...
    for income in [24000, 97000, 197000, 207000, 208000, 1E6]:
        federal, state, rate_federal_income, rate_federal_lcg, rate_state = tax(year, inflation, income)
        mylist.append({'income':round(income), 'federal':round(federal), 'state':round(state), 'total':round(federal+state),
                       'rate_federal_income':rate_federal_income, 'rate_federal_lcg':rate_federal_lcg, 
